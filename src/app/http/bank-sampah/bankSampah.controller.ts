...
import {
  AcceptOrRejectRequestDto,
  CreateCompleteRequestDto,
  SearchTrashBankPickupRequestDto,
} from 'src/app/validator/bank-sampah/bankSampah.dto';
...
export class BankSampahController {
    ...
    try {
      ...
      if (trashTypeIds.length !== weights.length) {
        throw new BadRequestException(
          'Trash type IDs and weights arrays must have the same length',
        );
      }
      ...
      throw new BadRequestException(
        'Failed to process trash acceptance: ' + error.message,
      );
    }
  }
  @UseGuards(JwtAuthGuard)
  ...
  async managePickupRequest(
    ...
    dto.status = status;
    const result =
      await this.bankSampahService.acceptOrRejectPickupRequest(dto);
    return {
      status: 'success',
  ...
  async fetchPickupRequest(
    ...
    const { page = 1, limit = 10, status,orderBy } = searchTrashBankPickupRequestDto;
